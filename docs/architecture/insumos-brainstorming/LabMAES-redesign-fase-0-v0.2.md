...
## Fase 0 — Arquitetura de informação e modelo de conteúdo v0.2

**Status:** modelo preliminar validado — pronto para especificação técnica do CMS  
...
12. Manter uma categoria `other` / “Outros” para produções, como escape controlado e não como classificação padrão.
13. Preservar egressos no site e relacioná-los historicamente a projetos, produções e eventos.
14. Modelar pesquisa, ensino e extensão como tipos de uma única entidade `project`.
15. Não pressupor migração automática de uma base de produções existente: o novo repositório será construído progressivamente a partir de inventário e catalogação.
...
Egressos permanecem como registros públicos históricos. A saída do laboratório não remove a pessoa nem suas relações anteriores; apenas altera seu `status` e, quando conhecido, registra `membershipEnd`.
...
preferredName:
roles: []
status: current | alumnus | collaborator
membershipStart:
membershipEnd:
bio:
photo:
...
Abrange pesquisa, ensino, extensão e projetos transversais em uma única entidade. A distinção é feita por `projectType`, permitindo páginas e filtros específicos sem duplicar estrutura, templates ou regras editoriais.
...
title:
acronym:
projectType: research | teaching | extension | transversal | other
...
A navegação pública poderá agrupar as produções em cinco macro categorias:
...
Internamente, `productionType` continuará mais específico para permitir filtros e metadados adequados. Tipos iniciais:
...
`other` / “Outros” é uma válvula de escape. Quando selecionado no CMS, deverá exigir um campo `otherTypeLabel` para registrar o tipo real. Se um mesmo rótulo começar a se repetir, ele deverá ser promovido a tipo oficial da taxonomia em revisão posterior.
...
license:
featured: false
catalogStatus: identified | verified | complete
sourceNotes:
lastVerifiedAt:
translations:
```
...
## 7. Arquitetura de informação preliminar v0.2
...
11. campo “Outros” condicionado à justificativa/rótulo específico;
12. status de vínculo para pessoas (`current`, `alumnus`, `collaborator`);
13. tipos de projeto em select controlado (`research`, `teaching`, `extension`, `transversal`, `other`);
14. estado de catalogação das produções (`identified`, `verified`, `complete`) visível aos editores.

---

## 10.1 Estratégia de construção do repositório de produções

Não existe hoje uma base única e organizada das produções do LabMAES. Portanto, a implantação não será tratada como uma “migração de dados”, mas como uma **catalogação institucional progressiva**.

Princípios:

1. não tentar completar todo o histórico antes de lançar a nova plataforma;
2. começar com um conjunto pequeno, confiável e representativo;
3. registrar a origem de cada informação durante a catalogação;
4. distinguir item apenas identificado de item bibliograficamente verificado;
5. permitir complementação contínua por bolsistas sem quebrar o schema;
6. não publicar metadados duvidosos como se fossem confirmados.

### Estados de catalogação

- `identified` — sabemos que a produção existe, mas o registro ainda pode estar incompleto;
- `verified` — autoria, título, ano e dados principais foram conferidos em fonte confiável;
- `complete` — o registro contém os metadados previstos aplicáveis ao seu tipo, arquivos/links e relações relevantes.

### Primeira carga recomendada

A primeira versão pública do repositório deverá priorizar:

1. produções recentes e já facilmente verificáveis;
2. produções diretamente vinculadas a projetos atuais;
3. materiais produzidos pelo próprio LabMAES;
4. livros, periódicos e produções com DOI/ISBN/URL institucional;
5. produções relacionadas ao Caminhos do Contemporâneo que já estejam documentadas.

Depois disso, o histórico poderá ser ampliado por ciclos de catalogação.

### Proveniência

Durante o backoffice editorial, cada registro poderá conter `sourceNotes` e `lastVerifiedAt`. Esses campos são internos e não precisam aparecer na interface pública, mas preservam rastreabilidade durante a construção do acervo.
...
### Encerramento da Fase 0 — entregáveis imediatos

Antes de alterar a interface pública, a Fase 0 deverá produzir:

- schema v1 das entidades principais;
- vocabulários controlados iniciais;
- matriz de relações;
- arquitetura de informação validada;
- especificação `.pages.yml` do Pages CMS;
- ADRs arquiteturais;
- política editorial mínima para cadastro e revisão;
- plano de catalogação inicial das produções.
...
## 17. Registro de decisões — v0.2

Decisões confirmadas nesta revisão:

- a categoria pública “Outros” será mantida para produções, mas com uso controlado;
- egressos permanecerão no histórico do laboratório e poderão ter páginas próprias;
- pesquisa, ensino e extensão serão tipos de uma única entidade `project`;
- não existe uma base consolidada de produções a migrar;
- o repositório de produções será formado progressivamente, com estados de catalogação e rastreabilidade interna;
- a próxima especificação técnica deverá focar o schema v1 e a configuração inicial do Pages CMS.
